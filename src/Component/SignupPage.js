...
function SignupPage() {
  ...
  return (
    ...
    <div id="signup_page_main">
        ...
        <div  className="container">
          <div id="sign_up_page_form" className="signup-container">
            ...
            <form id="form" onSubmit={handleSubmitCompany}>
              ...
              <div>
      <label htmlFor="countrySelect" className="text-[20px] font-bold">Select a country:</label>
      <select id="countrySelect" value={selectedCountry} onChange={handleCountryChange} >
        <option value="">Select</option>
        {countries.map((country) => (
          <option key={country.code} name="country"   value ={country.id }  >
            {country.name} 
          </option>
        ))}
      </select>
      ...
      <label htmlFor="countrySelect" className="text-[20px] font-bold">Select an Area:</label>
      <select id="countrySelect" value={selectedState} onChange={handleStateChange}  >
        <option value="">Select</option>
        {STATE.map((state) => (
          <option key={state.id} name="service_location_id"  value={state.id} >
            {state.name}
          </option>
        ))}
      </select>
      {selectedState && <p>Selected State: {selectedState}</p>}
              ...
              <div className="form-group ">
              <label for="profilePicture" className="text-[20px] font-bold">Company Logo:</label>
              <input type="file" id="profilePicture" name="profilePicture" value={credentials.profile} onChange={handleChange} accept="image/*"/>
              </div>
              <button type="submit">Sign Up as {userType}</button>
            </form>
          </div>
